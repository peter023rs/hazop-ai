# Stage 3 — AI HAZOP Reasoner (`hazop.s3_are`)
...
|---|---|---|
| Stage 1 topology graph | `mock_data/pump_vessel.py` | `hazop.s2_pml` adapter over `data/l1_output/plant_model_dexpi.json` — real 2401 unit: 428 nodes / 705 connections, ~26% verified flow direction |
| Stage 2 KB / RAG retriever | `reasoner/mock_retriever.py` | `hazop.s4_kb` — curated corpus + hybrid retrieval; wrap with `as_l3_retriever()` |
...
```bash
python -m hazop.s3_are.demo        # full pipeline on the mock process
python -m hazop.s3_are.evaluate    # score against the gold set (StubLLM)
pytest tests/s3_are                         # no network, no API key

# cloud model (optional): pip install -e ".[llm]" + export ANTHROPIC_API_KEY
python -m hazop.s3_are.evaluate --llm anthropic
# Stage B critic in the eval loop: lexical (default) | anthropic | local | none
python -m hazop.s3_are.evaluate --evidence-critic anthropic
...
python -m hazop.s3_are.evaluate --llm local --evidence-critic local --workers 4
...
`hazop.s2_pml.demo` part C runs THIS reasoner on the real plant graph
